# from: https://github.com/phillipdupuis/pydantic-to-typescript/blob/5bac13237e3b5d00a43a67aa4ce38db3de6eed04/pydantic2ts/cli/script.py
...
def clean_output_file(output_filename: str) -> None:
    ...
    for i, line in enumerate(lines):
        # add line breaks before export statements
    ...
    if len(lines) == 1:
        # this is a case where the output file is empty
...
def generate_json_schema(models: List[Type[BaseModel]]) -> str:
    ...
    try:
        ...
        try:
            ...
            print('failed to generate schema for models', models)
            raise
        ...
        pass
        # we may not need to recover the original state of the model_config, as the models will not be used elsewhere
...
def generate_typescript_defs(
    ...
    if not models:
        ...
        models = extract_pydantic_models(m)

    if include_func:
        models = [m for m in models if include_func(m)]

    exclude = set(exclude) | set(getattr(m, "_pydantic2ts_exclude", []))
    if exclude:
        models = [m for m in models if m.__name__ not in exclude]

    # if not models:
    #     logger.warn(f'No models, skip')
    #     return
    ...
    logger.info(f"Converting JSON schema to typescript definitions... {schema_file_path}")
